...
namespace Sitecore.SharedSource.DataImporter.Processors.Field
{
    public class UpdateLinks
    {
        ...
        public UpdateLinks(IDataMap dataMap, ILogger logger)
        {
            DataMap = dataMap;
            Logger = logger;
        ...
        public void Run(Item processor, Item itemToProcess, Item fieldMapping)
        {
            ...
            UpdateFields(fieldName, internalDomains, processor);
        }
        
        public void UpdateFields(string fieldName, List<string> internalDomains, Item processor)
        {
            ...
            {
                Item[] items = UrlImportMap.MaintainHierarchy ? UrlImportMap.ImportLocation.Axes.GetDescendants() : UrlImportMap.ImportLocation.Children.ToArray();
                Item linkItem = null;
                ...
                foreach (var item in items)
                {
                    ...
                    foreach (HtmlNode node in doc.DocumentNode.Descendants("a"))
                    {
                        ...
                        if (linkItem != null)
                        {
                            ...
                            Logger.Log($"Update Links: From: {olderValue} To: {newValue}", item.Paths.FullPath, Providers.LogType.Info, fieldName);
                        }
                    }
